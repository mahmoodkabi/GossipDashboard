...
namespace GossipDashboard.Controllers
{
    public class AmazingController : Controller
    {
        ...
        public AmazingController(string path, string domain, string ip)
        {
            this.path = path;
            this.domain = domain;
            this.ip = ip;
        ...
        public JsonResult ReadContentCategory()
        {
            //ایجاد  تگ آرتیکل به ازای هر پست
            var repo = new PostRepository();
            var postQuiz = repo.SelectPostByCategory("amazing").OrderByDescending(p => p.PostID).Take(200)
                .Select(p => new VM_Post
                {
                    Subject1 = p.Subject1,
                    PostID = p.PostID,
            ...
            return Json(postQuiz, JsonRequestBehavior.AllowGet);
        }
    }
}